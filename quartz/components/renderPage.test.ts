...
describe("renderTranscludes", () => {
  test("resolves a single page transclusion", () => {
    ...
    assert.ok(
      texts.includes("Target content"),
      "transcluded content should be inlined",
    );
  });
  ...
  test("allows the same page to be embedded twice as siblings", () => {
    const root: Root = {
      type: "root",
      children: [
        makeTranscludeBlockquote("target"),
        makeTranscludeBlockquote("target"),
      ],
    };
    ...
    assert.ok(
      firstText.includes("Duplicated content"),
      "first embed should resolve",
    );
    assert.ok(
    ...
    assert.ok(
      !secondText.includes("Circular transclusion"),
      "should not show circular warning",
    );
  });
  ...
  test("allows different sections of the same page to be embedded", () => {
    ...
    const secondText = JSON.stringify(second.children);
    assert.ok(
      firstText.includes("Intro text"),
      "first header section should resolve",
    );
    assert.ok(
    ...
    assert.ok(
      secondText.includes("Details text"),
      "second header section should resolve",
    );
    assert.ok(
      !secondText.includes("Circular transclusion"),
      "should not show circular warning",
  ...
  test("detects actual circular transclusion (A -> B -> A)", () => {
    ...
    const allFiles = [
      makePageData("pageA", pageA_htmlAst),
      makePageData("pageB", pageB_htmlAst),
    ];
    ...
    assert.ok(
      fullText.includes("Page B content"),
      "page B content should be inlined",
    );
    assert.ok(
      fullText.includes("Circular transclusion"),
      "circular A->B->A should be detected",
    );
    assert.ok(
      !fullText.includes("Page A content"),
      "page A should not be re-inlined inside B",
    );
  });
  ...
  test("self-referencing transclusion is blocked", () => {
    ...
    assert.ok(
      text.includes("Circular transclusion"),
      "self-reference should be blocked",
    );
  });
});
...
describe("pageResources", () => {
  ...
  test("uses baseDir prefix for resource paths in production mode", () => {
    ...
    const externalJs = result.js.find(
      (j) => j.contentType === "external" && "src" in j,
    );
  ...
  test("contentIndex path reflects baseDir", () => {
    ...
    const inlineJs = withPrefix.js.find(
      (j) => j.contentType === "inline" && "script" in j,
    );
    ...
    const inlineJsServe = withoutPrefix.js.find(
      (j) => j.contentType === "inline" && "script" in j,
    );

...
async function cloneWithSubdirAsync({ url, ref, subdir, pluginDir }) {
  ...
  try {
    ...
    if (!fs.existsSync(subdirPath)) {
      throw new Error(
        `Subdirectory "${subdir}" not found in cloned repository`,
      );
    }
...
function linkPeerPlugins(pluginDir) {
  ...
  for (const peerName of Object.keys(peers)) {
    const peerNodeModulesPath = path.join(
      pluginDir,
      "node_modules",
      ...peerName.split("/"),
    );
    ...
    const target = path.relative(
      path.dirname(peerNodeModulesPath),
      hostPeerPath,
    );
    trySymlink(target, peerNodeModulesPath);
  }
...
function isOverridableExport(name, dtsContent) {
  ...
  const declPattern = new RegExp(
    `declare\\s+const\\s+${declName}\\s*:\\s*(.+?)(?:;|$)`,
    "m",
  );
  ...
  for (const match of exportMatches) {
    ...
    for (const name of names) {
      ...
      if (
        ...
        !INTERNAL_EXPORTS.has(cleanName)
      ) {
...
async function regeneratePluginIndex() {
  ...
  for (const pluginName of pluginDirs) {
    ...
    const passthrough = named.filter(
      (n) => !isOverridableExport(n, dtsContent),
    );
...
export async function handlePluginInstallUnified({
  ...
  if (clean && latest) {
    console.log(
      styleText("red", "✗ --clean and --latest cannot be used together"),
    );
    return;
  }
  ...
  const resolvedConcurrency = Math.max(
    1,
    concurrencyOption ?? os.cpus().length,
  );
  ...
  if (dryRun && latest) {
    ...
    const nameWidth =
      Math.max(6, ...rows.map((row) => row.displayName.length)) + 2;
    ...
    if (isTTY) {
      ...
      for (const row of rows) {
        if (row.entry.commit === "local") {
          console.log(
            renderRow(row, "local", "—", styleText("green", "local")),
          );
          continue;
        }
        console.log(
    ...
    const promises = rows.map((row, index) => {
      ...
      const lsRemoteRef = row.entry.ref
        ? `refs/heads/${row.entry.ref}`
        : "HEAD";
        ...
        const color =
          status === "up to date" || status === "local"
            ...
            : status === "check failed"
              ...
              : "yellow";
        console.log(
          renderRow(rows[index], installed, latest, styleText(color, status)),
        );
      }
    }
    return;
  ...
  if (fromConfig) {
    ...
    const orphans = Object.keys(lockfile.plugins).filter(
      (name) => !configNames.has(name),
    );
    ...
    if (missing.length === 0) {
      console.log(
        styleText("green", "✓ All configured plugins are already installed"),
      );
      if (dryRun) {
        if (orphans.length > 0) {
          console.log();
          console.log(
            `Found ${orphans.length} orphaned plugin(s) in lockfile:\n`,
          );
          for (const name of orphans) {
            console.log(
              `  ${styleText("yellow", name)} — in lockfile but not in config`,
            );
          }
          console.log();
          console.log(
            styleText(
              "cyan",
              "Dry run — no changes made. Re-run without --dry-run to resolve.",
            ),
          );
        }
        return;
    ...
    if (missing.length > 0) {
      ...
      for (const entry of missing) {
        const name = extractPluginName(entry.source);
        console.log(
          `  ${styleText("yellow", name)} — ${formatSource(entry.source)}`,
        );
      }
      console.log();
      ...
      try {
        ...
        if (fs.existsSync(pluginDir)) {
          if (local) {
            console.log(
              styleText(
                "yellow",
                `⚠ ${name} directory already exists, updating lockfile`,
              ),
            );
        ...
        if (local) {
          ...
          if (!fs.existsSync(resolvedPath)) {
            console.log(
              styleText("red", `✗ Local path does not exist: ${resolvedPath}`),
            );
            failed++;
            continue;
          }
          console.log(
            styleText("cyan", `→ Linking ${name} from ${resolvedPath}...`),
          );
        ...
        console.log(
          styleText(
            "red",
            `✗ Failed to resolve ${formatSource(entry.source)}: ${error}`,
          ),
        );
        failed++;
      }
          ...
          try {
            if (subdir) {
              console.log(
                styleText(
                  "cyan",
                  `→ Cloning ${name} from ${url} (subdir: ${subdir})...`,
                ),
              );
              fs.mkdirSync(path.dirname(pluginDir), { recursive: true });
              ...
              console.log(
                styleText(
                  "green",
                  `✓ Cloned ${name}@${commit.slice(0, 7)} (subdir: ${subdir})`,
                ),
              );
            } else {
              console.log(
                styleText("cyan", `→ Cloning ${name} from ${url}...`),
              );

              const branchArg = ref ? ` --branch ${ref}` : "";
              await execAsync(
                `git clone --depth 1${branchArg} "${url}" "${pluginDir}"`,
              );
              ...
              lockfileChanged = true;
              console.log(
                styleText("green", `✓ Cloned ${name}@${commit.slice(0, 7)}`),
              );
            }
          } catch (error) {
            console.log(
    ...
    if (lockfileChanged) {
      ...
      if (failed === 0) {
        console.log(
          styleText("green", `✓ Resolved ${installed.length} plugin(s)`),
        );
      } else {
        console.log(
          styleText(
            "yellow",
            `⚠ Resolved ${installed.length} plugin(s), ${failed} failed`,
          ),
        );
      }
  ...
  if (dryRun) {
    ...
    console.log(
      styleText("cyan", "→ Dry run: plugins to install from lockfile..."),
    );
    for (const [name, entry] of entries) {
      const sourceLabel = entry.source
        ...
        : entry.resolved;
      const commitLabel =
        entry.commit === "local" ? "local" : entry.commit.slice(0, 7);
      console.log(
        `  ${styleText("yellow", name)} — ${sourceLabel} (${commitLabel})`,
      );
    }
    return;
  }
  ...
  if (clean) {
    ...
    for (const [name, entry] of entries) {
      ...
      if (fs.existsSync(pluginDir)) {
        console.log(
          styleText("yellow", `⚠ ${name}: directory exists, skipping`),
        );
        continue;
      }
        ...
        try {
          if (!fs.existsSync(entry.resolved)) {
            console.log(
              styleText(
                "red",
                `  ✗ ${name}: local path missing: ${entry.resolved}`,
              ),
            );
            failed++;
            continue;
          ...
          console.log(
            styleText("red", `✗ ${name}: failed to restore local symlink`),
          );
          failed++;
        }
        continue;
        ...
        async ({ name, entry, pluginDir }) => {
          try {
            if (entry.subdir) {
              ...
              await execAsync(
                `git clone --depth 1${branchArg} "${entry.resolved}" "${pluginDir}"`,
              );
    ...
    if (failed === 0) {
      ...
      console.log(
        styleText(
          "yellow",
          `⚠ Restored ${installed} plugin(s), ${failed} failed`,
        ),
      );
    }
    return;
  ...
  if (latest) {
    const pluginsToUpdate = nameFilter
      ? Array.from(nameFilter)
    ...
    for (const name of pluginsToUpdate) {
      ...
      if (!fs.existsSync(pluginDir)) {
        console.log(
          styleText(
            "yellow",
            `⚠ ${name} directory missing. Run 'npx quartz plugin install'.`,
          ),
        );
        continue;
      }
      ...
      await runParallel(
        ...
        async ({ name, pluginDir, entry }) => {
          try {
            ...
            if (entry.subdir) {
              ...
              if (newCommit !== entry.commit) {
                ...
                console.log(
                  styleText(
                    "gray",
                    `✓ ${name} rebuilt (subdir: ${entry.subdir})`,
                  ),
                );
              }
            } else {
              ...
              const resetTarget = entry.ref
                ...
                : "origin/HEAD";
              await execAsync(
                `git fetch --depth 1 origin${fetchRef ? " " + fetchRef : ""}`,
                {
                  cwd: pluginDir,
                },
              );
              ...
              if (newCommit !== entry.commit) {
                ...
                console.log(
                  styleText(
                    "green",
                    `✓ Updated ${name} to ${newCommit.slice(0, 7)}`,
                  ),
                );
              } else {
            ...
            console.log(
              styleText("red", `✗ Failed to update ${name}: ${error}`),
            );
          }
        },
      );
    ...
    if (entry.commit === "local") {
      try {
        if (fs.existsSync(pluginDir)) {
          ...
          if (
            stat.isSymbolicLink() &&
            fs.readlinkSync(pluginDir) === entry.resolved
          ) {
            console.log(
              styleText("gray", `  ✓ ${name} (local) already linked`),
            );
            installed++;
            continue;
          }
      ...
      if (entry.subdir) {
        ...
        if (currentCommit === entry.commit && !needsBuild(pluginDir)) {
          console.log(
            styleText(
              "gray",
              `  ✓ ${name}@${entry.commit.slice(0, 7)} already installed`,
            ),
          );
          installed++;
          continue;
  ...
  if (gitEntries.length > 0) {
    ...
    await runParallel(
      gitEntries,
      concurrency,
      async ({ name, entry, pluginDir, action }) => {
        try {
          if (action === "update") {
            console.log(
              styleText(
                "cyan",
                `  → ${name}: updating to ${entry.commit.slice(0, 7)}...`,
              ),
            );
            ...
            if (entry.subdir) {
              console.log(
                styleText(
                  "cyan",
                  `  → ${name}: cloning (subdir: ${entry.subdir})...`,
                ),
              );
              fs.mkdirSync(path.dirname(pluginDir), { recursive: true });
              await cloneWithSubdirAsync({
            ...
            console.log(
              styleText("green", `  ✓ ${name}@${entry.commit.slice(0, 7)}`),
            );
            pluginsToBuild.push({ name, pluginDir });
            installed++;
          }
          ...
          console.log(
            styleText(
              "red",
              `  ✗ ${name}: failed to ${action === "update" ? "update" : "clone"}`,
            ),
          );
          failed++;
        }
  ...
  if (failed === 0) {
    ...
    console.log(
      styleText(
        "yellow",
        `⚠ Installed ${installed} plugin(s), ${failed} failed`,
      ),
    );
  }
}
...
export async function handlePluginAdd(
  ...
  if (nameOverride && sources.length > 1) {
    console.log(
      styleText(
        "red",
        "✗ --name/--as can only be used when adding a single plugin",
      ),
    );
    return;
  }
  if (subdirOverride && sources.length > 1) {
    console.log(
      styleText(
        "red",
        "✗ --subdir can only be used when adding a single plugin",
      ),
    );
    return;
  }
    ...
    try {
      ...
      if (fs.existsSync(pluginDir)) {
        console.log(
          styleText(
            "yellow",
            `⚠ ${name} already exists. Use 'update' to refresh.`,
          ),
        );
        continue;
      }
      ...
      if (local) {
        ...
        console.log(
          styleText(
            "cyan",
            `→ Adding ${name} from local path ${resolvedPath}...`,
          ),
        );
        fs.mkdirSync(path.dirname(pluginDir), { recursive: true });
        symlinkOrCopySync(resolvedPath, pluginDir);
      ...
      console.log(
        styleText("red", `✗ Failed to add ${formatSource(source)}: ${error}`),
      );
    }
  }
        ...
        try {
          if (subdir) {
            console.log(
              styleText(
                "cyan",
                `→ Adding ${name} from ${url} (subdir: ${subdir})...`,
              ),
            );
            fs.mkdirSync(path.dirname(pluginDir), { recursive: true });
            const commit = await cloneWithSubdirAsync({
            ...
            console.log(
              styleText(
                "green",
                `✓ Added ${name}@${commit.slice(0, 7)} (subdir: ${subdir})`,
              ),
            );
          } else {
            ...
            addedPlugins.push({ name, pluginDir, source, configSource });
            console.log(
              styleText("green", `✓ Added ${name}@${commit.slice(0, 7)}`),
            );
          }
        } catch (error) {
          console.log(
            styleText(
              "red",
              `✗ Failed to add ${formatSource(source)}: ${error}`,
            ),
          );
        }
      },
    ...
    for (const { pluginDir, source, configSource } of addedPlugins) {
      ...
      if (manifest?.components) {
        ...
        if (
          ...
          layoutPositions.has(comp.defaultPosition)
        ) {
          newEntry.layout = {
...
export async function handlePluginEnable(names) {
  ...
  if (!json) {
    console.log(
      styleText("red", "✗ No quartz.config.yaml found. Cannot enable plugins."),
    );
    return;
  }
  ...
  for (const name of names) {
    const entry = json.plugins.find(
      (e) =>
        extractPluginName(e.source) === name || formatSource(e.source) === name,
    );
    if (!entry) {
      console.log(
        styleText(
          "yellow",
          `⚠ Plugin "${name}" not found in quartz.config.yaml`,
        ),
      );
      continue;
    }
...
export async function handlePluginDisable(names) {
  ...
  if (!json) {
    console.log(
      styleText(
        "red",
        "✗ No quartz.config.yaml found. Cannot disable plugins.",
      ),
    );
    return;
  }
...
export async function handlePluginConfig(name, options = {}) {
  ...
  if (!entry) {
    console.log(
      styleText("red", `✗ Plugin "${name}" not found in quartz.config.yaml`),
    );
    return;
  }
  ...
  if (options.set) {
    ...
    writePluginsJson(json);
    console.log(
      styleText("green", `✓ Set ${name}.${key} = ${JSON.stringify(value)}`),
    );
  } else {
...
export async function handlePluginStatus() {
  ...
  const nameWidth =
    Math.max(8, ...rows.map((row) => row.displayName.length)) + 2;
  const sourceWidth =
    Math.max(8, ...rows.map((row) => row.sourceLabel.length)) + 2;
  const commitWidth =
    Math.max(6, ...rows.map((row) => row.commitLabel.length)) + 2;
  ...
  const promises = rows.map((row, index) => {
    ...
    return execAsync(`git ls-remote "${row.entry.resolved}" ${lsRemoteRef}`)
      .then(({ stdout }) => {
        const latestCommit = stdout.split("\t")[0].trim();
        const status =
          latestCommit === row.entry.commit ? "up_to_date" : "update_available";
  ...
  if (updatesAvailable.length > 0) {
    ...
    console.log(
      styleText("gray", "Run 'npx quartz plugin install --latest' to update."),
    );
  }
...
export async function handlePluginPrune({ dryRun = false } = {}) {
  ...
  if (orphans.length === 0) {
    console.log(
      styleText("green", "✓ No orphaned plugins found — nothing to prune"),
    );
    return;
  }
  ...
  if (dryRun) {
    console.log(
      styleText(
        "cyan",
        "Dry run — no changes made. Re-run without --dry-run to prune.",
      ),
    );
    return;
  }

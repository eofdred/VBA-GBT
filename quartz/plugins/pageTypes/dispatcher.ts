...
function getPageTypes(ctx: BuildCtx): QuartzPageTypePluginInstance[] {
  return (ctx.cfg.plugins.pageTypes ??
    []) as unknown as QuartzPageTypePluginInstance[];
}
...
export const PageTypeDispatcher: QuartzEmitterPlugin<
  Partial<DispatcherOptions>
> = (userOpts) => {
  ...
  return {
    ...
    async *emit(ctx, content, resources) {
      const pageTypes = [...getPageTypes(ctx)].sort(
      ...
      const allFilesWithVirtual = [
        ...allFiles,
        ...virtualEntries.map((ve) => ve.vfile.data),
      ];

      // Render Body components to populate htmlAst for transclusion
      populateVirtualPageHtmlAst(
        virtualEntries,
        ctx,
        allFilesWithVirtual,
        resources,

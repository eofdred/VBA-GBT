...
function detectPluginType(
  ...
  const hasPageTypeProps = ["match", "body", "layout"].every(
    (key) => key in mod,
  );

  const hasTransformerProps = [
    "textTransform",
    "markdownPlugins",
    "htmlPlugins",
  ].some(
    (key) =>
      key in mod && (typeof mod[key] === "function" || mod[key] === undefined),
  );
  ...
  const hasEmitterProps = ["emit"].some(
    (key) => key in mod && typeof mod[key] === "function",
  );
...
async function resolveSinglePlugin(
    ...
    return {
      plugin: {
        ...
        manifest: {
          ...
          description:
            specifier.manifest?.description ?? "Inline plugin instance",
  ...
  if (pluginSource === "git") {
    try {
      ...
      const categoryOrCategories =
        manifest.category ?? detectPluginType(module);
      ...
      const processingType: ProcessingCategory | undefined = Array.isArray(
        categoryOrCategories,
      )
        ? (categoryOrCategories.find((c) =>
            (processingCategories as readonly string[]).includes(c),
          ) as ProcessingCategory | undefined)
        : (processingCategories as readonly string[]).includes(
              categoryOrCategories,
            )
      ...
      const loadedPlugin: LoadedPlugin = {
        ...
        type: detectedType,
        source: gitSpec.local
          ? `local:${gitSpec.repo}`
          : `${gitSpec.repo}#${gitSpec.ref}`,
      };
  ...
  try {
    ...
    const categoryOrCategories =
      manifest.category ?? detectPluginType(importedModule);
...
export async function resolvePlugins(
  ...
  if (options.verbose) {
    console.log(
      styleText("cyan", `Resolving ${specifiers.length} external plugin(s)...`),
    );
  }
...
export function instantiatePlugin<T>(
  ...
  const factory = loadedPlugin.plugin as (
    opts?: T,
  ) => ReturnType<typeof loadedPlugin.plugin>;

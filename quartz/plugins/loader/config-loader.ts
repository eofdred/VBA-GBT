...
async function readManifestFromPackageJson(
  source: PluginSource,
): Promise<PluginManifest | null> {
  try {
    const gitSpec = parsePluginSource(source);
...
export async function loadQuartzConfig(
      ...
      if (isComponentOnly) {
        ...
        try {
          ...
          if (typeof module.init === "function") {
            const initOverrides = componentRegistry.getOptionOverrides(
              gitSpec.name,
            );
            const options = {
        ...
        try {
          ...
          const hasFrames =
            manifest?.frames && Object.keys(manifest.frames).length > 0;
          if (hasComponents) {
            await loadComponentsFromPackage(gitSpec.name, manifest);
          }
          if (hasFrames) {
  ...
  const instantiate = async (
    ...
    for (const { entry, manifest } of items) {
      try {
        ...
        const pluginOverrides = componentRegistry.getOptionOverrides(
          gitSpec.name,
        );
        const options = {
          ...manifest?.defaultOptions,
  ...
  const plugins: PluginTypes = {
    ...
    pageTypes: [
      ...(await instantiate(pageTypes, "pageType")),
      ...builtinPageTypes,
    ],
  };
...
export async function loadQuartzLayout(layoutOverrides?: {
  ...
  if (layoutConfig.byPageType) {
    for (const [pageType, override] of Object.entries(
  ...
  if (footerEntry) {
    ...
    const footerReg =
      componentRegistry.get("footer") ?? componentRegistry.get("Footer");
    if (footerReg) {
      if (
        typeof footerReg.component === "function" &&
        !("displayName" in footerReg.component)
      ) {
  ...
  if (layoutOverrides?.byPageType) {
    for (const [pageType, overrideLayout] of Object.entries(
      layoutOverrides.byPageType,
    )) {
...
function buildLayoutForEntries(
  ...
  for (const entry of entries) {
    ...
    if (
      typeof reg.component === "function" &&
      !("displayName" in reg.component)
    ) {
...
function resolveGroups(
  ...
  for (const item of items) {
    if (item.group) {
      ...
      entries.push({
        priority: groupPriority.get(item.group) ?? 50,
...
function applyDisplayWrapper(
  ...
  if (display === "mobile-only") {
    const MobileOnly = require("../../components/MobileOnly")
      .default as Function;
    return MobileOnly(component) as QuartzComponent;
  } else {
    const DesktopOnly = require("../../components/DesktopOnly")
      .default as Function;
    return DesktopOnly(component) as QuartzComponent;
  }
}

function applyConditionWrapper(
  component: QuartzComponent,
  ...
  const ConditionalRender = require("../../components/ConditionalRender")
    .default as Function;

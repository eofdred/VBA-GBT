...
export function parsePluginSource(source: PluginSource): GitPluginSpec {
  ...
  if (source.startsWith("github:")) {
    ...
    if (!owner || !repo) {
      throw new Error(
        `Invalid GitHub source: ${source}. Expected format: github:user/repo`,
      );
    }
...
function buildInstalledPlugin(
  ...
  try {
    ...
    console.error(
      styleText("red", `✗`),
      `${name}: post-install build failed: ${message}`,
    );
...
export async function installPlugin(
  ...
  if (spec.local) {
    ...
    if (!options.force && fs.existsSync(pluginDir)) {
      ...
      try {
        ...
        if (
          stat.isSymbolicLink() &&
          fs.realpathSync(pluginDir) === fs.realpathSync(spec.repo)
        ) {
          if (options.verbose) {
            console.log(
              styleText("cyan", `→`),
              `Plugin ${spec.name} already linked`,
            );
          }
    ...
    if (options.verbose) {
      console.log(
        styleText("cyan", `→`),
        `Linking ${spec.name} from ${spec.repo}...`,
      );
    }
    ...
    if (spec.subdir) {
      ...
      if (fs.existsSync(pkgPath)) {
        if (options.verbose) {
          console.log(
            styleText("cyan", `→`),
            `Plugin ${spec.name} already installed`,
          );
        }
        return { pluginDir, nativeDeps: collectNativeDeps(pluginDir) };
      }
  ...
  if (spec.subdir) {
    ...
    if (!fs.existsSync(subdirPath)) {
      fs.rmSync(tmpDir, { recursive: true });
      throw new Error(
        `Subdirectory "${spec.subdir}" not found in repository ${spec.repo}`,
      );
    }
...
export async function installPlugins(
  ...
  for (const source of sources) {
    try {
      const spec =
        typeof source === "string" ? parsePluginSource(source) : source;
  ...
  for (const pluginName of plugins) {
    ...
    try {
      ...
      if (options.verbose) {
        console.error(
          styleText("yellow", `⚠`),
          `Skipping ${pluginName}: Not a git repo`,
        );
      }
    }
  }
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (!fs.existsSync(distIndex)) {
      if (options.verbose) {
        console.log(
          styleText("yellow", `⚠`),
          `Skipping ${pluginName}: no dist/index.d.ts found`,
        );
      }
      continue;
    }

...
async function buildQuartz(argv: Argv, mut: Mutex, clientRefresh: () => void) {
  ...
  const pluginNames = (
    key: "transformers" | "filters" | "emitters" | "pageTypes",
...
async function rebuild(
  ...
  try {
    ...
    const dispatcher = cfg.plugins.emitters.find(
    ...
    const contentWithVirtual =
      ctx.virtualPages.length > 0
        ? [...processedFiles, ...ctx.virtualPages]
        : processedFiles;
